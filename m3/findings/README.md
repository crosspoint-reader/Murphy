...
- NVS contains readable WiFi configuration, including SSID/password-like values. Treat `m3/extracted/nvs.bin` as sensitive.
- Ghidra 12.1 has been set up with Xtensa support and a project was created at `m3/ghidra-project/MurphyM3`.
...
- `m3/findings/display_driver.md`: OEM driver dataflow, init sequence, frame/refresh path, LUT tables (default + alternate), grayscale bench-tuning, and current AA-disabled shipping state.
- `m3/findings/touch.md`: OEM touch strings, controller candidates, custom probe results, and SDK integration plan.
- `m3/findings/button_input.md`: physical key map, OEM long-press/custom-key evidence, and combo test plan.
- `m3/findings/front_light.md`: OEM front-light settings evidence, external panel lead, confirmed `GPIO48` PWM control, and SDK implementation notes.
- `m3/findings/sd_card_gpio.md`: OEM-recovered Murphy 4-bit SDMMC mapping, superseded SPI lead, and next probe code.
- `m3/findings/usb_logging.md`: confirmed stock USB app logs, capture workflow, and verbosity/debug-flag search notes.
- `m3/findings/online_research.md`: web-sourced product, panel, and module clues.
- `m3/findings/audio.md`: headphone-jack/audio playback evidence, codec unknowns, and porting impact.
- `m3/findings/clock_rtc.md`: clock, NTP, alarms, internal RTC evidence, and external RTC test plan.
- `m3/findings/network_and_ui.md`: web endpoints, OTA, WiFi/weather UI.
- `m3/findings/storage_and_nvs.md`: NVS and filesystem observations.
- `m3/findings/ghidra.md`: Ghidra/Xtensa setup and analysis artifacts.
- `m3/findings/oem_headless_ghidra.md`: headless Ghidra commands, OEM hardware-mining scripts, and current static-analysis limits.
- `m3/findings/porting_crosspoint.md`: feasibility and work breakdown for porting `community-sdk` and `crosspoint-reader-main`.
- `m3/analysis/`: raw extracted partitions, strings, segments, Ghidra logs.
- `m3/binwalk_extracted/esp-partitions/`: requested binwalk-style extraction directory populated with ESP partitions.
